...
import { CUSTODY_TOOLS_SCHEMA, CustodyToolHandler } from "./custody-tools.ts";
...
export class MemoryToolHandler {
  ...
  async handleToolCall(tool: MCPToolCall): Promise<MCPToolResult> {
    try {
      ...
      console.error(
        `[mcp v2.3] Tool execution failed for ${tool.name}:`,
        error,
      );
      return {
        content: [{
  ...
  private async handleMemoryRead(args: any): Promise<MCPToolResult> {
    ...
    const context = await this.client.readMemoryPacket(
      clientId,
      args.__access_token,
    );
  ...
  private async handleMemoryWrite(args: any): Promise<MCPToolResult> {
    ...
    const result = await this.client.writeSessionBack(
      writeback,
      args.__access_token,
    );
  ...
  private async handleMemorySearch(args: any): Promise<MCPToolResult> {
    ...
    const filters = {
      content_type: typeof args.content_type === "string"
        ? args.content_type
        : undefined,
      domain: typeof args.domain === "string" ? args.domain : undefined,
    };

    const { results, mode, ordinal_match } = await this.client
      .searchRecordsHybrid(
        query,
        clientId,
        limit,
        ...
        text: JSON.stringify(
          {
            ...
            statistics: {
              ...
              total_artifacts: stats.locked_decisions + stats.ns_records +
                stats.session_writebacks,
            },
            timestamp: stats.timestamp,
            anti_amnesia_status: stats.locked_decisions > 0
              ? "active"
              : "initializing",
          },
          null,
  ...
  private async handleMemoryRecall(args: any): Promise<MCPToolResult> {
    ...
    const clientId = args.client_id || "viralbrain";
    const { answer, citations, sources } = await this.client.recallMemory(
      question,
      clientId,
      args.__access_token,
    );
  ...
  private async handleMemoryGetLatestSession(
    args: any,
  ): Promise<MCPToolResult> {
    const clientId = args.client_id || "viralbrain";
    const records = await this.client.getLatestSession(
      clientId,
      "active",
      args.__access_token,
    );
  ...
  private async handleMemoryGetBySession(args: any): Promise<MCPToolResult> {
    ...
    if (sessionNumber === undefined || sessionNumber === null) {
      throw new Error(
        "session_number parameter is required for memory_get_by_session",
      );
    }

    const clientId = args.client_id || "viralbrain";
    const records = await this.client.getBySession(
      clientId,
      sessionNumber,
      undefined,
      args.__access_token,
  ...
  private async handleMemoryFilter(args: any): Promise<MCPToolResult> {
    ...
    const records = await this.client.filterRecords(clientId, {
      ...
      attributes: args.attributes && typeof args.attributes === "object"
        ? args.attributes
        : undefined,
      title_ilike: typeof args.title_ilike === "string"
        ? args.title_ilike
        : undefined,
      body_ilike: typeof args.body_ilike === "string"
        ? args.body_ilike
        : undefined,
      since: typeof args.since === "string" ? args.since : undefined,
  ...
  private async handleMemoryCurrentSession(args: any): Promise<MCPToolResult> {
    const clientId = args.client_id || "viralbrain";
    const sessionNumber = await this.client.currentSession(
      clientId,
      args.__access_token,
    );
  ...
  private async handleSearchWrapper(args: any): Promise<MCPToolResult> {
    ...
    const { results } = await this.client.searchRecordsHybrid(
      query,
      clientId,
      limit,
      filters,
        ...
        text: JSON.stringify(
          {
            results: results.map((r: any) => ({
              id: String(r.id),
              title: r.title ||
                `${r.content_type} — ${r.source_session ?? "unknown"}`,
              text: typeof r.body === "string" ? r.body : "",
              url:
                `https://udafklielwqdppnagtwc.supabase.co/rest/v1/ns_records?id=eq.${r.id}`,
            })),
          },
          null,
          2,
  ...
  private async handleGetRecordById(args: any): Promise<MCPToolResult> {
    const recordId = typeof args?.record_id === "string"
      ? args.record_id.trim()
      : "";
    if (!recordId) {
    ...
    const record = await this.client.getRecordById(
      recordId,
      args.client_id || "viralbrain",
      args.__access_token,
    );
  ...
  private async handleFetchWrapper(args: any): Promise<MCPToolResult> {
    if (isRecordId(args?.id)) {
      return await this.handleGetRecordById({
        record_id: String(args.id).trim(),
        client_id: args.client_id,
        __access_token: args.__access_token,
      });
    }
        ...
        text: JSON.stringify(
          {
            ...
            url:
              `https://udafklielwqdppnagtwc.supabase.co/functions/v1/neuralsync-context`,
            metadata: {
              session_number: context.session_number,
              locked_decisions: context.locked_decisions_count,
              memory_records: context.memory_records_count,
...
export const MEMORY_TOOLS_SCHEMA = [
  {
    name: "memory_read",
    description:
      "Retrieve memory packet context for session continuity. Gets latest session writebacks, locked decisions, and memory records.",
    annotations: {
      readOnlyHint: true,
    },
    inputSchema: {
      ...
      properties: {
        ...
        session_number: {
          type: "number",
          description:
            "Target session number (optional, auto-increments if not provided)",
        },
      },
    },
  },
  {
    name: "memory_write",
    description:
      "Capture session outcomes to Memory Packets via the canonical smart-close write path. Stores session writebacks (ns_session_writebacks), locks critical decisions (ns_locked_decisions), and creates typed memory records (ns_records).",
    annotations: {
      readOnlyHint: false,
    },
    inputSchema: {
      ...
      properties: {
          ...
          items: {
            ...
            properties: {
              title: {
                type: "string",
                description:
                  "Short decision title (becomes decision_title in ns_locked_decisions; unique per client_id)",
              },
              rationale: {
                type: "string",
              ...
              decision_type: {
                type: "string",
                description:
                  "Optional category (e.g. technical, product_framing, architectural_discovery, implementation, session_close, data_calibration, diagnostic)",
              },
              priority: {
                type: "number",
                description:
                  "Optional priority 0-9 (lower=higher priority; default 5)",
              },
              confidence_score: {
                type: "number",
                description:
                  "Optional confidence 0.0-1.0 decimal (default 0.95)",
              },
              platform: {
                type: "string",
                description:
                  "Optional platform tag (e.g. farsight, neuralsynch, cross-platform)",
              },
            },
          ...
          items: {
            ...
            properties: {
              ...
              language: {
                type: "string",
                description:
                  "Optional language (typescript, python, sql, markdown, etc.)",
              },
            },
            required: ["file_path"],
            additionalProperties: true,
          ...
          items: {
            ...
            properties: {
              ...
              status: {
                type: "string",
                description:
                  "Optional status (canonical: active, open, resolved, deprecated, superseded, recurring; common variants pending/failed/blocked/in_progress/closed/archived are normalized server-side)",
              },
              error_type: {
                type: "string",
                description:
                  "Optional category (e.g. validation, api, infrastructure, schema, transport)",
              },
            },
        ...
        next_session_tasks: {
          type: "array",
          description:
            "Carry-forward tasks for the next session. Free-form objects.",
          items: {
            type: "object",
            additionalProperties: true,
          },
        ...
        carry_forward_context: {
          type: "string",
          description:
            "Long-form context to carry into the next session (optional)",
        },
        bolt_project: {
          type: "string",
  ...
  },
  {
    name: "memory_search",
    description:
      'Hybrid FTS + vector search across the typed memory substrate (ns_records). Combines full-text search with cosine-similarity ranking on embeddings. Optional filters narrow to a specific content_type or domain. v2.1 surfaces ordinal_match for "Decision N" queries.',
    annotations: {
      readOnlyHint: true,
    },
    inputSchema: {
      ...
      properties: {
        ...
        query: {
          type: "string",
          description:
            'Search query to find relevant memories. Special pattern: "Decision N" triggers fast ordinal lookup before hybrid search.',
        },
        limit: {
          type: "number",
        ...
        content_type: {
          type: "string",
          description:
            "Filter to one content type. Valid values: session_writeback, decision, code_artifact, error, schema_snapshot, spec, prompt, runbook, asset_image, asset_chat, prose. Optional.",
        },
        domain: {
          type: "string",
          description:
            "Filter to one domain (e.g. neuralsynch.memory-architecture, farsight.predict, cross-platform). Optional.",
        },
      },
      required: ["query"],
    },
  },
  {
    name: "memory_stats",
    description:
      "Get memory system statistics and health metrics for the client.",
    annotations: {
      readOnlyHint: true,
    },
    inputSchema: {
  ...
  // ─── S250 recall + retrieval tools (read-only, client_id server-injected) ──
  {
    name: "memory_recall",
    description:
      'Synthesizing recall over the typed memory substrate (ns_records). Embeds the question (Voyage voyage-4-lite, FTS fallback), runs the recency-aware hybrid RPC, and synthesizes an answer (Anthropic Haiku) strictly from the returned records. Returns { answer, citations, sources }; answers "This is not yet in your IP store." when the substrate has no match. Mirrors the direct-path neuralsync-query function for cross-path parity.',
    annotations: {
      readOnlyHint: true,
    },
    inputSchema: {
      ...
      properties: {
        question: {
          type: "string",
          description:
            "Natural-language question to answer from NeuralSynch memory.",
        },
        client_id: {
          type: "string",
          description: "Client identifier (default: viralbrain)",
  ...
  },
  {
    ...
    description:
      "Get the most recent typed memory records (ns_records) for the client, newest first. Optional filters narrow to a content_type, domain, or records since a timestamp.",
    annotations: {
      readOnlyHint: true,
    },
    inputSchema: {
      ...
      properties: {
        content_type: {
          type: "string",
          description:
            "Filter to one content type (optional). E.g. decision, session_writeback, code_artifact, error, runbook.",
        },
        domain: {
          type: "string",
          description:
            "Filter to one domain (optional). E.g. neuralsynch.memory-architecture, farsight.predict.",
        },
        since: {
          type: "string",
          description:
            "ISO 8601 timestamp; only records created at or after this time (optional).",
        },
        limit: {
          type: "number",
  ...
  },
  {
    ...
    description:
      "Get the latest session row for the client (highest session number, active by default).",
    annotations: {
      readOnlyHint: true,
    },
    inputSchema: {
  ...
  },
  {
    ...
    description:
      "Get all typed memory records (ns_records) attached to a specific session number for the client.",
    annotations: {
      readOnlyHint: true,
    },
    inputSchema: {
  ...
  },
  {
    name: "memory_filter",
    description:
      "Structured filter over typed memory records (ns_records). Combine content_type, domain, status, tags, JSON attributes, title/body ILIKE substrings, and a created-at window, then order and limit. All filters are optional.",
    annotations: {
      readOnlyHint: true,
    },
    inputSchema: {
      ...
      properties: {
        ...
        status: {
          type: "string",
          description:
            "Filter to one status (optional). E.g. active, resolved, deprecated, superseded.",
        },
        tags: {
          type: "array",
          description:
            "Filter to records carrying all of these tags (optional).",
          items: {
            type: "string",
          },
        },
        attributes: {
          type: "object",
          description:
            "Filter by JSON attributes (optional). Free-form key/value object matched against the record attributes column.",
          additionalProperties: true,
        },
        title_ilike: {
          type: "string",
          description:
            "Case-insensitive substring match against the record title (optional). Plain substring; no % wildcards needed.",
        },
        body_ilike: {
          type: "string",
          description:
            "Case-insensitive substring match against the record body (optional).",
        },
        since: {
          type: "string",
          description:
        ...
        until: {
          type: "string",
          description:
            "ISO 8601 timestamp; only records created at or before this time (optional).",
        },
        order: {
          type: "string",
          description:
            "Ordering hint (optional). E.g. created_at.desc, created_at.asc.",
        },
        limit: {
          type: "number",
  ...
  // ─── ChatGPT Deep Research / Company Knowledge compatibility wrappers ──
  {
    name: "search",
    description:
      "Search NeuralSynch memory for records matching a query. Returns a list of matches with id, title, and text content. Compatible with ChatGPT Deep Research conventions.",
    annotations: {
      readOnlyHint: true,
    },
    inputSchema: {
  ...
  },
  {
    name: "fetch",
    description:
      "Fetch a single NeuralSynch memory packet by id. Returns the full context including session continuity, locked decisions, and memory records. Compatible with ChatGPT Deep Research conventions.",
    annotations: {
      readOnlyHint: true,
    },
    inputSchema: {
      ...
      properties: {
        id: {
          type: "string",
          description:
            "Packet identifier. For NeuralSynch this is the client_id (default: viralbrain).",
        },
      },
      required: ["id"],
    },
    ...
    inputSchema: {
      ...
      properties: {
        record_id: {
          type: "string",
          description:
            "ns_records record UUID (as returned in the id field by `search`)",
        },
      },
      required: ["record_id"],
    },
