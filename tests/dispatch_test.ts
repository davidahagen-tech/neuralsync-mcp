...
import { ALL_TOOLS_SCHEMA, MemoryToolHandler, isRecordId } from '../memory-tools.ts';
...
Deno.test('isRecordId accepts a UUID and rejects a client_id', () => {
  ...
  assert(isRecordId(`  ${REAL_UUID}  `), 'surrounding whitespace should be tolerated');
  ...
  assert(!isRecordId(`prefix-${REAL_UUID}`), 'must be anchored, not a substring match');
  assert(!isRecordId(`${REAL_UUID}-suffix`), 'must be anchored, not a substring match');
});
...
Deno.test('every advertised tool has a dispatch case — none falls through', async () => {
  ...
  const stub = stubFetch([
    ...
    ['/functions/v1/neuralsync-custody', { ok: true, result: { verified: true, version_id: 'v' } }],
    ['/functions/v1/retrieve-context-packet', { context_prompt: 'x', session_number: 1 }],
    ['/rest/v1/rpc/', []],
  ]);
  ...
  try {
    for (const tool of ALL_TOOLS_SCHEMA as any[]) {
      ...
      if (tool.name === 'search' || tool.name === 'memory_search') args.query = 'q';
      ...
      if (tool.name === 'custody_resolve_alias') args.alias = 'project:x/y/current';
      if (tool.name.startsWith('custody_') && tool.name !== 'custody_resolve_alias') {
        args.version_id = 'f509c795-367c-46d4-af79-536ae7f8d402';
      }
      if (tool.name === 'memory_get_by_session') args.session_number = 1;
      if (tool.name === 'memory_write') { args.session_number = 1; args.objective = 'o'; }

      const res = await handler.handleToolCall({ name: tool.name, arguments: args });
...
Deno.test('get_record_by_id returns the FULL body, not an excerpt', async () => {
  ...
  try {
    const res = await new MemoryToolHandler().handleToolCall({
      ...
      arguments: { record_id: REAL_UUID },
    });
    const parsed = JSON.parse(res.content[0].text);
    assertEquals(parsed.found, true);
    assertEquals(parsed.id, REAL_UUID);
    assertEquals(parsed.body.length, RECORD_ROW.body.length, 'body was truncated');
...
Deno.test('get_record_by_id fails CLOSED when the record does not exist', async () => {
  ...
  try {
    ...
    const parsed = JSON.parse(res.content[0].text);
    assertEquals(res.isError, true, 'a missing record must be an error, not a quiet success');
...
Deno.test('REGRESSION: fetch(UUID) returns the record, not an empty client packet', async () => {
  ...
  const stub = stubFetch([
    ...
    ['/functions/v1/retrieve-context-packet', {
      context_prompt: 'No locked decisions found. No active records in ns_records yet.',
      session_number: 0,
...
Deno.test('fetch(UUID) for a missing record fails closed rather than returning a packet', async () => {
  const stub = stubFetch([
    ['/rest/v1/ns_records', []],
    ['/functions/v1/retrieve-context-packet', { context_prompt: 'should never be reached' }],
  ]);
  try {
    const res = await new MemoryToolHandler().handleToolCall({
      name: 'fetch',
